...
<div class="row">
	<?php echo CHtml::link('<i class="fa fa-plus fa-fw"></i>Tambah Jadwal', array('arc/create'), array('class'=>'btn btn-sm btn-success pull-right plus-right-margin'));?>
</div>
...
//		'flag',
		...
		array(
			...
			'viewButtonUrl'=>function($data){
				return Yii::app()->controller->createUrl("/arc/view",array("id"=>$data["id"]));
			},
			'updateButtonUrl'=>function($data){
				return Yii::app()->controller->createUrl("/arc/update",array("id"=>$data["id"]));
			},
			'deleteButtonUrl'=>function($data){
				return Yii::app()->controller->createUrl("/arc/delete",array("id"=>$data["id"]));
			},
		),
	),
)); ?>
